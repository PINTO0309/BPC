...
from .model import BPC, ModelConfig

LOGGER = logging.getLogger("bpc")

LABEL_MAP = {0: "not_plain", 1: "plain"}
...
def _parse_image_size_arg(raw: Any) -> tuple[int, int]:
    ...
    raise argparse.ArgumentTypeError(
        "Image size must be specified as a single integer (e.g. '48') or as 'HEIGHTxWIDTH' (e.g. '48x48')."
    )
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for idx in range(images.size(0)):
                ...
                pred_class_id = 1 if prob >= 0.5 else 0
                results.append(
                    {
                        "path": batch["path"][idx],
                        ...
                        "prob_plain": prob,
                        "pred_label": LABEL_MAP[pred_class_id],
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    train_dataset = BPCDataset(train_samples, transform=train_transform)
    val_dataset = BPCDataset(splits["val"], transform=eval_transform) if splits["val"] else None
    test_dataset = BPCDataset(splits["test"], transform=eval_transform) if splits["test"] else None
    ...
    model = BPC(model_config).to(device)
    base_metadata = {
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"bpc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "bpc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"bpc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "bpc_best_", 10)
...
def predict_images(
    ...
    image_size_raw = normalization.get("image_size", (48, 48))
    try:
...
def export_to_onnx(
    ...
    torch.onnx.export(
        ...
        output_path,
        input_names=["images"],
        output_names=["prob_plain"],
        dynamic_axes={"images": {0: "batch"}, "prob_plain": {0: "batch"}},
        opset_version=opset,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="BPC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the BPC binary classifier from parquet data.")
    ...
    train_parser.add_argument(
        ...
        help="Square size (e.g. 48) or HEIGHTxWIDTH (e.g. 48x48) for resizing input images.",
    )
