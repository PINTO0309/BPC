...
# Classes are fixed: 0 = not_plain, 1 = plain.
CLASS_MAP = {
    "not_plain": 0,
    "plain": 1,
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build dataset.parquet for the BPC classifier from image folders under data/, "
            "embedding resized image bytes and deriving classes from not_plain/plain path components."
        )
    )
    parser.add_argument(
        "--root",
        type=Path,
        default=Path("data"),
        help="Root directory containing not_plain/ and plain/ image folders.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def encode_resized_image(path: Path, image_size: tuple[int, int]) -> bytes:
    ...
    with Image.open(path) as image:
        resized = image.convert("RGB").resize((width, height), RESAMPLING.LANCZOS)
...
def build_dataframe(root: Path, extensions: Iterable[str], image_size: tuple[int, int]) -> pd.DataFrame:
    ...
    if df.empty:
        raise RuntimeError(
            f"No labelled images found under {root}; expected images below not_plain/ or plain/ directories."
        )
    df = df.sort_values(["split", "class_id", "image_path"]).reset_index(drop=True)
    if skipped_unlabeled:
        print(f"Skipped {skipped_unlabeled} image(s) without not_plain/plain path component.")
    return df
