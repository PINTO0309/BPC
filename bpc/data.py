...
class Sample:
    """Lightweight container describing a single BPC image."""
...
POSITIVE_LABELS = {"plain"}
NEGATIVE_LABELS = {"not_plain"}
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "plain" if label == 1 else "not_plain"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'plain').",
            split,
            total,
...
class BPCDataset(Dataset):
    """Torch dataset for BPC images on disk or embedded bytes."""
