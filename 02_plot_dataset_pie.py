#!/usr/bin/env python3
"""Generate a class-ratio pie chart PNG from the BPC parquet dataset."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a pie chart PNG for class composition in the BPC parquet dataset."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--title",
        default=None,
        help="Optional chart title. Defaults to a BPC dataset title.",
    )
...
def main() -> None:
    ...
    title = args.title or f"BPC dataset class ratio ({args.input.name})"
    ax.set_title(
        f"{title}\nTotal: {total:,} samples",
        fontsize=14,
        pad=18,
